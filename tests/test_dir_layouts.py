...
def test_anat_in_each_session_folder():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        second_session_folder = Path(tmpdir) / "anat_in_each_session_folder" / "sub-01" / "ses-second"
        ...
        for file in second_session_folder.glob("pet/*"):
            shutil.move(
                file,
                second_session_folder / "pet" / file.name.replace("ses-baseline_", "ses-second_"),
            )
        
        for file in second_session_folder.glob("anat/*"):
            shutil.move(
                file,
                second_session_folder / "anat" / file.name.replace("ses-baseline_", "ses-second_"),
            )
...
def test_anat_in_subject_folder():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        original_anat_folder = Path(tmpdir) / "anat_in_subject_folder" / "sub-01" / "ses-baseline" / "anat"    
